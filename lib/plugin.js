        ...
        if (ext === '.css') {
          ...
          if (secondExt === '.rtl') {
            for (var value in _this.assetsMap) {
              _this.assetsMap[value][_this.chunkName]['rtl.css'] = '' + compilation.outputOptions.publicPath + assetName;
            }
          } else {
            for (var _value in _this.assetsMap) {
              _this.assetsMap[_value][_this.chunkName][ext.replace('.', '')] = '' + compilation.outputOptions.publicPath + assetName;
            }
          }
        } else {
          for (var _value2 in _this.assetsMap) {
            _this.assetsMap[_value2][_this.chunkName][ext.replace('.', '')] = '' + compilation.outputOptions.publicPath + assetName;
          }
        }
      }
    });
